...
export default function ViewAllQuickPicksPage() {
  ...
  if (isLoading) {
    return (
      <div 
        className={`min-h-screen pb-20 overflow-y-auto scrollbar-hide ${
          'bg-background'
        }`}
        style={{ 
          scrollbarWidth: 'none',
          ...
          <div className="grid gap-4">
            {filteredItems.map((item) => (
              <Card key={item.id} className="hover:shadow-md transition-shadow rounded-xl">
                <CardContent className="p-4">
                  <div className="flex items-center space-x-4">
                    <div className="w-16 h-16 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex-shrink-0 flex items-center justify-center overflow-hidden">
                      {item.imageUrl ? (
                        <img 
                          src={item.imageUrl} 
                          alt={item.name}
