...
interface TrendingConfigPanelProps {
  canteenId?: string;
}

export function TrendingConfigPanel({ canteenId }: TrendingConfigPanelProps) {
  const [activeInnerTab, setActiveInnerTab] = useState<"all" | "trending" | "preview">("all");
  ...
  if (menuItemsLoading) {
    return (
      <OwnerCard
        title="🔥 Trending Items Configuration"
        description="Configure which menu items appear in the Trending Now section."
  ...
  return (
    ...
    >
      {/* Inner Tabs */}
      <Tabs value={activeInnerTab} onValueChange={(v) => setActiveInnerTab(v as "all" | "trending" | "preview")} className="flex-1 flex flex-col min-h-0 overflow-hidden">
